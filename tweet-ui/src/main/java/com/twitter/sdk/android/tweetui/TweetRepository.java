...
class TweetRepository {
    ...
    void update(final long tweetId, final String replyText, final Double latitude, final Double longitude, final String placeId, final String mediaIds, final Callback<Tweet> cb) {
        userAuthQueue.addClientRequest(new Callback<TwitterApiClient>() {
            @Override
            public void success(Result<TwitterApiClient> result) {
                result.data.getStatusesService().update(replyText, tweetId, false, latitude, longitude, placeId, false, false, mediaIds, cb);
            }
